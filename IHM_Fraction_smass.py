...
def calculate_statistics(df, property_to_bin, property_to_calculate):
    ...
    stellar = np.log10(np.array(property_to_bin))
    ...
    for i in range(len(bin_edges)):
       ...
       w = np.where((stellar >= min_bin) & (stellar < max_bin) & (ihsfraction > 0))[0] 
...
def plot_IHMFraction_vs_smass(df, property_1, property_2, titles, save_filename):
    ...
    for idx, (df, title) in enumerate(zip(df, titles)):
        ...
        mean_values, std_values, median_values, mid_bin_values, error_mean_values = calculate_statistics(df, df['smass'], df['IHM_Fraction'])
...
def IHM_Fraction(save_filename):
    ...
    mean_values, std_values, median_values, mid_bin_values, error_mean_values = calculate_statistics(df, df_calculated['smass'], df_calculated['IHM_Fraction'])
    ...
    plt.scatter(np.log10(df_diluted['smass']), df_diluted['IHM_Fraction'], alpha=0.1, s=0.8, color='gray')
